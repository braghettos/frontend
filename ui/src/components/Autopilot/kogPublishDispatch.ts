...
export const dispatchKogPublish = async (
...
): Promise<{ compiled: PublishCompileResult; deepLink: string | null; follow: PublishFollowSeed | null }> => {
  ...
  const gateProbe: ApplyResourceSetOp[] | undefined = resolution.held
    ...
    : undefined
  if (!restDefTarget) {
    return { compiled: { denial: 'publish cancelled — destination not confirmed', ops: null }, deepLink: null, follow: null }
  }
  if (resolution.missingOasDocument) {
    return { compiled: { denial: 'denied — the previewed mapping uses a configmap:// oasPath but no OpenAPI document is attached; paste the document in the rail first (it is held client-side and committed at publish), or preview a URL oasPath.', ops: null }, deepLink: null, follow: null }
  }
  if (!resolution.held) {
    return { compiled: { denial: 'denied — no previewed RestDefinition to publish (previewRestDef a mapping first)', ops: null }, deepLink: null, follow: null }
  }
  if (ctx.publishViaClaim) {
    ...
    return { compiled: res.compiled, deepLink: res.deepLink, follow: res.follow }
  }
  const built = buildKogPublishAsPrOps(targeted, resolution.held)
  if (built.length > MAX_APPLY_SET_OPS) {
    return { compiled: { denial: `denied — the KOG publish set has ${built.length} ops, over the ${MAX_APPLY_SET_OPS}-op cap.`, ops: null }, deepLink: null, follow: null }
  }
  return { compiled: compileKogPublishOps(built, ctx.previewGate.evaluate(gateProbe), ctx.origin), deepLink: null, follow: null }
}
